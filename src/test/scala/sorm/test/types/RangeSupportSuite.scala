...
class RangeSupportSuite extends FunSuite with ShouldMatchers with MultiInstanceSuite {
  ...
  instancesAndIds foreach { case (db, dbId) =>
    ...
    test(dbId + " - saved entities are correct"){
      db.fetchById[A](a1._id.get).a should equal (2 to 4)
      db.fetchById[A](a2._id.get).a should equal (9 to 1)
    }
    ...
    test(dbId + " - 0 to 0 range"){
      ...
      db.fetchById[A](a._id.get).a should equal (0 to 0)
    }
...
object RangeSupportSuite {
  ...
  case class A ( a : Range ) extends Persistable
